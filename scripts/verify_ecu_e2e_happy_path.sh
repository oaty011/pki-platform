...
echo
echo "== 6) ECU revoke =="
REVOKE_RESPONSE="$(api_post "${BASE_URL}/ecu-certificates/revoke" "{\"deviceId\":\"${DEVICE_ID}\",\"certSerial\":\"${CERT_SERIAL}\",\"issuerId\":\"${ISSUER_ID}\"}")"
printf '%s\n' "$REVOKE_RESPONSE"
...
echo
echo "== 8) ECU recover =="
RECOVER_RESPONSE="$(api_post "${BASE_URL}/ecu-certificates/recover" "{\"deviceId\":\"${DEVICE_ID}\",\"certSerial\":\"${CERT_SERIAL}\",\"issuerId\":\"${ISSUER_ID}\"}")"
printf '%s\n' "$RECOVER_RESPONSE"
