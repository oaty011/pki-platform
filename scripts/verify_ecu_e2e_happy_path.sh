...
echo
echo "== 4) First ECU current query =="
...
CORE_ACTIVE_IS_CURRENT_AFTER_SYNC="$(sql_scalar "SELECT COALESCE(CAST(is_current AS text), '') FROM pki_ecu.${CORE_ACTIVE_TABLE} WHERE cert_serial = '$(sql_escape "$CERT_SERIAL")' AND issuer_id = '$(sql_escape "$ISSUER_ID")' LIMIT 1;")"
...
CORE_ACTIVE_IS_CURRENT_AFTER_RECOVER="$(sql_scalar "SELECT COALESCE(CAST(is_current AS text), '') FROM pki_ecu.${CORE_ACTIVE_TABLE} WHERE cert_serial = '$(sql_escape "$CERT_SERIAL")' AND issuer_id = '$(sql_escape "$ISSUER_ID")' LIMIT 1;")"
...
echo
echo "== 10) Second ECU current query =="
