...
DEVICE_ID="${DEVICE_ID:-ecu-e2e-$(date +%s)}"
ECU_TEMPLATE_ID="${ECU_TEMPLATE_ID:-ecu-template-demo}"
REQUEST_ID_PREFIX="${REQUEST_ID_PREFIX:-ecu-e2e}"
...
echo
echo "== 1) Generate ECU key and CSR =="
openssl genrsa -out "$CSR_KEY_PATH" 2048 >/dev/null 2>&1 || fail "failed to generate ECU private key"
openssl req -new -sha256 \
  -key "$CSR_KEY_PATH" \
  -subj "/CN=${DEVICE_ID}/OU=ignored-by-platform/O=ignored/C=CN" \
  -out "$CSR_PATH" >/dev/null 2>&1 || fail "failed to generate ECU CSR"
CSR_CONTENT="$(cat "$CSR_PATH")"
assert_non_empty "$CSR_CONTENT" "CSR content is empty"

echo
echo "== 2) ECU apply =="
APPLY_BODY="$(python3 -c 'import json,sys; print(json.dumps({"requestId": sys.argv[1], "templateId": sys.argv[2], "deviceId": sys.argv[3], "csr": open(sys.argv[4]).read()}))' "$REQUEST_ID" "$ECU_TEMPLATE_ID" "$DEVICE_ID" "$CSR_PATH")"
APPLY_RESPONSE="$(api_post "${BASE_URL}/ecu-certificates/apply" "$APPLY_BODY")"
printf '%s\n' "$APPLY_RESPONSE"
...
echo
echo "== 5) First ECU current query =="
CURRENT_QUERY_1="$(api_post "${BASE_URL}/ecu-certificates/current/query" "{\"deviceId\":\"${DEVICE_ID}\",\"certSerial\":\"\"}")"
printf '%s\n' "$CURRENT_QUERY_1"
assert_api_success "$CURRENT_QUERY_1" "first current query"
assert_equals "$DEVICE_ID" "$(printf '%s' "$CURRENT_QUERY_1" | json_get "data.subjectId")" "current query subjectId mismatch"
assert_equals "$ECU_ORGANIZATION" "$(printf '%s' "$CURRENT_QUERY_1" | json_get "data.organization")" "current query organization mismatch"
...
CORE_ACTIVE_EXISTS_AFTER_SYNC="$(sql_scalar "SELECT COUNT(*) FROM pki_ecu.${CORE_ACTIVE_TABLE} WHERE cert_serial = '$(sql_escape "$CERT_SERIAL")' AND issuer_id = '$(sql_escape "$ISSUER_ID")';")"
CORE_ACTIVE_NOT_AFTER_AFTER_SYNC="$(sql_scalar "SELECT COALESCE(CAST(not_after AS text), '') FROM pki_ecu.${CORE_ACTIVE_TABLE} WHERE cert_serial = '$(sql_escape "$CERT_SERIAL")' AND issuer_id = '$(sql_escape "$ISSUER_ID")' LIMIT 1;")"
...
echo
echo "== 7) ECU revoke =="
REVOKE_RESPONSE="$(api_post "${REVOCATION_BASE_URL}/ecu-certificates/revoke" "{\"deviceId\":\"${DEVICE_ID}\",\"certSerial\":\"${CERT_SERIAL}\",\"issuerId\":\"${ISSUER_ID}\"}")"
printf '%s\n' "$REVOKE_RESPONSE"
assert_api_success "$REVOKE_RESPONSE" "ECU revoke"
assert_equals "revoked" "$(printf '%s' "$REVOKE_RESPONSE" | json_get "data.action")" "revoke action mismatch"

echo
echo "== 8) SQL checks after revoke =="
CORE_ACTIVE_EXISTS_AFTER_REVOKE="$(sql_scalar "SELECT COUNT(*) FROM pki_ecu.${CORE_ACTIVE_TABLE} WHERE cert_serial = '$(sql_escape "$CERT_SERIAL")' AND issuer_id = '$(sql_escape "$ISSUER_ID")';")"
...
echo
echo "== 9) ECU recover =="
RECOVER_RESPONSE="$(api_post "${REVOCATION_BASE_URL}/ecu-certificates/recover" "{\"deviceId\":\"${DEVICE_ID}\",\"certSerial\":\"${CERT_SERIAL}\",\"issuerId\":\"${ISSUER_ID}\"}")"
printf '%s\n' "$RECOVER_RESPONSE"
assert_api_success "$RECOVER_RESPONSE" "ECU recover"
...
CORE_ACTIVE_EXISTS_AFTER_RECOVER="$(sql_scalar "SELECT COUNT(*) FROM pki_ecu.${CORE_ACTIVE_TABLE} WHERE cert_serial = '$(sql_escape "$CERT_SERIAL")' AND issuer_id = '$(sql_escape "$ISSUER_ID")';")"
CORE_ACTIVE_NOT_AFTER_AFTER_RECOVER="$(sql_scalar "SELECT COALESCE(CAST(not_after AS text), '') FROM pki_ecu.${CORE_ACTIVE_TABLE} WHERE cert_serial = '$(sql_escape "$CERT_SERIAL")' AND issuer_id = '$(sql_escape "$ISSUER_ID")' LIMIT 1;")"
...
NOT_AFTER_MATCH="$(sql_scalar "SELECT CASE WHEN (SELECT not_after FROM pki_issuance.certificate_issue_fact WHERE cert_serial = '$(sql_escape "$CERT_SERIAL")' AND issuer_id = '$(sql_escape "$ISSUER_ID")' ORDER BY created_at DESC LIMIT 1) = (SELECT not_after FROM pki_ecu.${CORE_ACTIVE_TABLE} WHERE cert_serial = '$(sql_escape "$CERT_SERIAL")' AND issuer_id = '$(sql_escape "$ISSUER_ID")' LIMIT 1) THEN 'true' ELSE 'false' END;")"
...
echo
echo "== 11) Second ECU current query =="
CURRENT_QUERY_2="$(api_post "${BASE_URL}/ecu-certificates/current/query" "{\"deviceId\":\"${DEVICE_ID}\",\"certSerial\":\"\"}")"
printf '%s\n' "$CURRENT_QUERY_2"
assert_api_success "$CURRENT_QUERY_2" "second current query"
assert_equals "$DEVICE_ID" "$(printf '%s' "$CURRENT_QUERY_2" | json_get "data.subjectId")" "second current query subjectId mismatch"
assert_equals "$ECU_ORGANIZATION" "$(printf '%s' "$CURRENT_QUERY_2" | json_get "data.organization")" "second current query organization mismatch"
...
echo
echo "[PASS] ECU E2E happy path verified"
