...
APP_ID="${APP_ID:-app-invalid-csr-$(date +%s)}"
...
VALID_KEY_PATH="${TEST_CSR_DIR}/app-invalid-csr.key.pem"
VALID_CSR_PATH="${TEST_CSR_DIR}/app-invalid-csr.csr.pem"
BROKEN_CSR_PATH="${TEST_CSR_DIR}/app-invalid-csr-broken.csr.pem"
...
echo
echo "== 1) Generate valid APP CSR and tampered CSR =="
openssl genrsa -out "$VALID_KEY_PATH" 2048 >/dev/null 2>&1 || fail "failed to generate APP private key"
openssl req -new -sha256 \
  -key "$VALID_KEY_PATH" \
  -subj "/CN=${APP_ID}/OU=ignored-by-platform/O=ignored/C=CN" \
  -out "$VALID_CSR_PATH" >/dev/null 2>&1 || fail "failed to generate APP CSR"
mutate_csr_signature "$VALID_CSR_PATH" "$BROKEN_CSR_PATH"

echo
echo "== 2) Empty CSR should fail =="
EMPTY_REQUEST_ID="$(request_id "${APP_ID}-empty")"
EMPTY_HTTP_CODE="$(api_post_capture "${BASE_URL}/app-certificates/apply" "{\"requestId\":\"${EMPTY_REQUEST_ID}\",\"templateId\":\"${APP_TEMPLATE_ID}\",\"appId\":\"${APP_ID}\",\"csr\":\"\"}" "$EMPTY_BODY_FILE")"
...
INVALID_REQUEST_ID="$(request_id "${APP_ID}-invalid")"
INVALID_HTTP_CODE="$(api_post_capture "${BASE_URL}/app-certificates/apply" "{\"requestId\":\"${INVALID_REQUEST_ID}\",\"templateId\":\"${APP_TEMPLATE_ID}\",\"appId\":\"${APP_ID}\",\"csr\":\"not a valid csr\"}" "$INVALID_BODY_FILE")"
...
BROKEN_REQUEST_ID="$(request_id "${APP_ID}-broken")"
BROKEN_APPLY_BODY="$(python3 -c 'import json,sys; print(json.dumps({"requestId": sys.argv[1], "templateId": sys.argv[2], "appId": sys.argv[3], "csr": open(sys.argv[4]).read()}))' "$BROKEN_REQUEST_ID" "$APP_TEMPLATE_ID" "$APP_ID" "$BROKEN_CSR_PATH")"
BROKEN_HTTP_CODE="$(api_post_capture "${BASE_URL}/app-certificates/apply" "$BROKEN_APPLY_BODY" "$BROKEN_BODY_FILE")"
BROKEN_BODY="$(cat "$BROKEN_BODY_FILE")"
printf '%s\n' "$BROKEN_BODY"
assert_business_failure "$BROKEN_HTTP_CODE" "$BROKEN_BODY" "csr signature verification failed"

echo
echo "[PASS] APP apply invalid CSR verification passed"
