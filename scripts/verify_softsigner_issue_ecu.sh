...
DEVICE_ID="${DEVICE_ID:-softsigner-ecu-$(date +%s)}"
ECU_TEMPLATE_ID="${ECU_TEMPLATE_ID:-ecu-tbox}"
REQUEST_ID_PREFIX="${REQUEST_ID_PREFIX:-softsigner-ecu}"
...
ISSUED_CERT_PATH="${TEST_CSR_DIR}/${REQUEST_ID}.issued.cert.pem"
...
echo
echo "== 1) Generate temporary ECU key and CSR =="
...
echo
echo "== 2) ECU apply with real CSR =="
...
[[ "$SUBJECT_TEXT" == *"$DEVICE_ID"* ]] || fail "subject does not contain deviceId"
[[ "$SUBJECT_TEXT" == *"TBOX"* ]] || fail "subject does not contain expected ECU OU"
[[ "$SUBJECT_TEXT" == *"DFMC ECU"* ]] || fail "subject does not contain DFMC ECU"
...
echo
echo "[PASS] soft signer ECU issuance verified"
