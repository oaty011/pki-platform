...
APP_ID="${APP_ID:-sync-concurrency-$(date +%s)}"
REQUEST_ID_PREFIX="${REQUEST_ID_PREFIX:-sync-concurrency}"
...
echo
echo "== 2) Resolve certSerial and issuerId =="
...
echo
echo "== 3) Concurrent sync-core-active =="
for i in $(seq 1 "$CONCURRENCY"); do
  (
    api_post "${BASE_URL}/certificates/sync-core-active/${REQUEST_ID}" '{}' \
...
echo
echo "== 5) SQL checks =="
SYNC_STATUS="$(sql_scalar "SELECT sync_status FROM pki_issuance.certificate_issue_fact WHERE request_id = '$(sql_escape "$REQUEST_ID")';")"
ROW_COUNT="$(sql_scalar "SELECT COUNT(*) FROM pki_app.${CORE_ACTIVE_TABLE} WHERE cert_serial = '$(sql_escape "$CERT_SERIAL")' AND issuer_id = '$(sql_escape "$ISSUER_ID")';")"
LATEST_ACTIVE_CERT_SERIAL="$(printf '%s' "$CURRENT_QUERY" | json_get "data.currentActiveCertificate.certSerial")"
LATEST_ACTIVE_ISSUER_ID="$(printf '%s' "$CURRENT_QUERY" | json_get "data.currentActiveCertificate.issuerId")"
...
assert_equals "done" "$SYNC_STATUS" "sync-core-active did not end in done"
assert_equals "1" "$ROW_COUNT" "sync-core-active did not leave exactly one target row"
assert_equals "$CERT_SERIAL" "$LATEST_ACTIVE_CERT_SERIAL" "default query should return the latest active certificate after concurrent sync"
assert_equals "$ISSUER_ID" "$LATEST_ACTIVE_ISSUER_ID" "default query latest active issuerId mismatch after concurrent sync"

echo
echo "[PASS] sync-core-active concurrency verification passed"
