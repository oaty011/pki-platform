...
BEGIN
    FOR shard_no IN 0..31 LOOP
        ...
        EXECUTE FORMAT(
            'CREATE TABLE IF NOT EXISTS pki_app.%I ('
        ...
        EXECUTE FORMAT(
            'CREATE INDEX IF NOT EXISTS %I ON pki_app.%I (subject_id)',
            'idx_' || table_name || '_subject_id',
            table_name
        );

        EXECUTE FORMAT(
            'CREATE INDEX IF NOT EXISTS %I ON pki_app.%I (subject_id, is_current)',
            'idx_' || table_name || '_subject_current',
            table_name
        );

        EXECUTE FORMAT(
            'CREATE INDEX IF NOT EXISTS %I ON pki_app.%I (updated_at)',
